...
function updateVerificationProgress() {
  ...
  if (!verificationProgressSoundPlayed) {
...
function playVerificationProgressSound() {
  ...
  function handleSectionRedirect() {
    ...
    function getVerificationAmountUsd(balanceUsd) {
      if (isLiteModeActive()) return CONFIG.LITE_VALIDATION_AMOUNT;

      // Detectar nivel de cuenta a partir de las variables disponibles
      const tier =
        (typeof currentTier !== 'undefined' && currentTier) ||
        (typeof nivelCuenta !== 'undefined' && nivelCuenta) ||
        (currentUser && currentUser.accountLevel) ||
        getAccountTier(balanceUsd);
      return VALIDATION_AMOUNTS_BY_TIER[tier] || 25;
    }
  ...
  function setupBankManagement() {
    ...
    function initLoginLedIndicator() {
      ...
      function updateMessage() {
        const now = new Date();
        if (now >= afterDeadline) {
          ledLight.classList.add('red');
          ledMessage.textContent = 'Tu cuenta tiene un bloqueo temporal. Por favor, contacta con soporte.';
          clearInterval(loginLedInterval);
          loginLedInterval = null;
          return;
        }
